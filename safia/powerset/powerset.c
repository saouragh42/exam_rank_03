...
void	print_subset(int *numbers, int *path, int len)
{
	int i = 0;
	int is_first = 1;
...
void	backtrack(int *numbers, int *path, int len, int index, int sum, int n)
{
	if (index == len)
	{
		if (sum == n)
			print_subset(numbers, path, len);
		return ;
	}
	path[index] = VISITED;
	backtrack(numbers, path, len, index + 1, sum + numbers[index], n);
	path[index] = EMPTY;
	backtrack(numbers, path, len, index + 1, sum, n);
}
...
int	main(int argc, char **argv)
{
	...
	backtrack(numbers, path, len, 0, 0, n);
	free(numbers);
	free(path);
	return (0);
}
